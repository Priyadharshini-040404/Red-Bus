...
void booked(string mob){
    ...
    if(o==1){
        cout<<"\n--- Old Bookings ---\n";
        for(auto &c:rows) if(isPastDate(c[4])) showRow(c);
        return; 
    } 
    else if(o==2){
        bool again=true;
        while(again){
            ...
            auto it=find_if(rows.begin(),rows.end(),[&](auto &r){return r[0]==id && !isPastDate(r[4]);});
            if(it==rows.end()){ cout<<"❌ Ticket not found or it’s old.\n"; }
            else {
                showRow(*it);
